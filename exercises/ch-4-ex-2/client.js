...
var client = {
  ...
  scope: "read write delete fetch"
};

var wordApi = "http://localhost:9002/words";
var wordApix = "http://localhost:9002/wordsx";
...
app.get("/words", function(req, res) {
  res.render("words", { words: "", word: "", timestamp: 0, result: "noget" });
  return;
});
...
app.get("/fetch_word", function(req, res) {
  var headers = {
    Authorization: "Bearer " + access_token,
    "Content-Type": "application/x-www-form-urlencoded"
  };

  var resource = request("GET", wordApix, { headers: headers });
  console.log("fetched");
...
app.get("/add_word", function(req, res) {
  ...
  if (resource.statusCode >= 200 && resource.statusCode < 300) {
    res.render("words", { words: "", word: "", timestamp: 0, result: "add" });
    return;
  } else {
    res.render("words", { words: "", word: "", timestamp: 0, result: "noadd" });
    return;
  }
});
...
app.get("/delete_word", function(req, res) {
  ...
  if (resource.statusCode >= 200 && resource.statusCode < 300) {
    res.render("words", { words: "", word: "", timestamp: 0, result: "rm" });
    return;
  } else {
    res.render("words", { words: "", word: "", timestamp: 0, result: "norm" });
    return;
  }
});
