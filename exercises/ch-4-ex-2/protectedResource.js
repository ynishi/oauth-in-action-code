...
app.get("/wordsx", getAccessToken, requireAccessToken, function(req, res) {
  console.log("fetch");
  if (__.contains(req.access_token.scope, "fetch")) {
    res.json({
      words: savedWords[savedWords.length - 1],
      timestamp: Date.now()
    });
  } else {
    res.set(
      "WWW-Authenticate",
      'Bearer realm=localhost:9002, error="insufficient_scope", scope="fetch"'
    );
    res.status(403).end();
  }
});
...
app.delete("/words", getAccessToken, requireAccessToken, function(req, res) {
  ...
  if (__.contains(req.access_token.scope, "delete")) {
    ...
    res.set(
      "WWW-Authenticate",
      'Bearer realm=localhost:9002, error="insufficient_scope", scope="save"'
    );
    res.status(403).end();
  }
});
